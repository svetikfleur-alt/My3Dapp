done: routine-ui — CAD interface polish (2026-05-11) — Modify toolbar group wrapped in ToolSplitGroup border; FeatureDialogButtonPrimary/Secondary hover+press states added (dark+light themes); SketchConstraintToolButton:disabled opacity state; Recipe→Expression label + monospace font in all 5 feature dialogs; Recipe tab renamed Script in MainWindow inspector; improved tooltips for Sweep/Loft/Fillet/Chamfer/Shell/LinearPattern/CircularPattern/Mirror; 8 files changed; build: dotnet not available on Linux (net10.0-windows target); commit pending
