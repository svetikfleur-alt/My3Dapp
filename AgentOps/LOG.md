inspector: 2026-05-14 — Fixed datum plane create/toggle/delete silently failing in CadProjectStore; fixed Extrude tooltip missing (E) shortcut; fixed Sweep tooltip and dialog subtitle describing plain extrusion as sweep; commit 15eb909. Reported: Sweep = twisted extrude (no guide path), Loft distance artificial, Fillet/Chamfer/Shell silently no-op on non-ExtrudeSolid, SketchRotateToolButton missing IsChecked binding, Sweep/Loft icons duplicate Extrude/Revolve icons.
