...
- Fix Point rendering (2026-05-10) - Increased THREE.js PointsMaterial size from 4/6px to 16/18px and SoftwareViewport markerSize from 3.2/3.8px to 6.0/7.0px; points now visible at CAD-appropriate size.

- Fix Line preview (2026-05-10) - Added cursor indicator point before first click: BuildLinePreview now returns [CadSketchPoint at cursor] when PendingLineStart is null, giving visual feedback before the anchor is set.

- Fix Rectangle preview (2026-05-10) - Added cursor indicator point before first anchor click: BuildRectanglePreview returns [CadSketchPoint at cursor] when PendingShapeAnchor is null; shows anchor point when cursor is at same position as anchor. Point tool preview wired: BuildPreviewEntities now dispatches CadSketchToolKind.Point to return cursor indicator.
