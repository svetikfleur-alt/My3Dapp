...
public sealed class CadProjectStore
{
    ...
    {
        var entityDof = session.DraftEntities.Sum(SketchEntityDof);

        var constraintDof = 0;
        foreach (var c in session.ManualConstraints)
        {
            constraintDof += c.Kind switch
            {
                ...
                CadSketchConstraintKind.Fixed =>
                    SketchEntityDof(session.DraftEntities.FirstOrDefault(e => c.EntityIds.Contains(e.Id))),
                _ => 0
            };
        }

        var dimDof = session.ManualDimensions.Count(d => !d.IsDriven);
        return entityDof - constraintDof - dimDof;
    }
    ...
    private static int SketchEntityDof(CadSketchEntity? entity) => entity switch
    {
        ...
        CadSketchSlot => 5,
        CadSketchPolygon => 3,
        CadSketchSpline spline => spline.ControlPointsXY.Count,
        _ => 0
    };
    ...
    private static List<CadSketchEntity> BuildPreviewEntities(CadSketchSession session, Vector2D point)
    {
        return session.ActiveTool switch
        {
            ...
            CadSketchToolKind.Point => [new CadSketchPoint { X = point.X, Y = point.Y }],
            _ => []
        };
    }
    ...
    private static List<CadSketchEntity> BuildLinePreview(CadSketchSession session, Vector2D point)
    {
        if (session.PendingLineStart is null)
        {
            return [new CadSketchPoint { X = point.X, Y = point.Y }];
        }
    ...
    private static List<CadSketchEntity> BuildRectanglePreview(CadSketchSession session, Vector2D point)
    {
        ...
        if (Distance(anchor, point) <= 0.001d)
        {
            return [new CadSketchPoint { X = anchor.X, Y = anchor.Y }];
        }
